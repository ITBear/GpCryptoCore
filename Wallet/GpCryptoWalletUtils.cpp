...
#include "../Keys/Curve25519/GpCryptoKeyFactory_Ed25519_HD.hpp"
#include "../Keys/Curve25519/GpCryptoKeyFactory_Ed25519_Rnd.hpp"
...
bool    GpCryptoWalletUtils::SValidateMnemonic (GpRawPtrCharR aMnemonic)
{
...
GpSecureStorage::CSP    GpCryptoWalletUtils::SSeedFromMnemonic
(
...
GpCryptoHDKeyStorage::CSP   GpCryptoWalletUtils::SGenerateBip44 (GpRawPtrByteR aSeed)
{
    //BIP44
...
GpCryptoKeyFactory::SP  GpCryptoWalletUtils::SNewHDKeyFactoryMnemonic (GpRawPtrCharR aMnemonic, GpRawPtrCharR aPassword)
{
    // Validate mnemonic
    ...
    GpCryptoHDKeyStorage::CSP bip44Root = SGenerateBip44(seed.VC().ViewR().R());
...
)
{
    ...
    {
        GpSecureStorageViewR    privateKeyStrHexViewR   = aPrivateKeyStrHex.VC().ViewR();
        GpRawPtrByteR           privateKeyStrHexBytesR  = privateKeyStrHexViewR.R();

        privateKey->Resize(privateKeyStrHexBytesR.SizeLeft() / 2_byte);
        StrOps::SToBytesHex(privateKeyStrHexBytesR.AsStringView(), privateKey->ViewRW().RW());
    }

    return SNewAddrFromPrivateKey(aAddrFactory, privateKey);
}
